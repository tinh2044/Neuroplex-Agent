...
<template>
  <div class="message-box" :class="[message.role, customClasses]">
    <p v-if="message.role === 'user' || message.role === 'sent'" class="message-text">
      {{ message.content }}
    </p>
    ...
    >
      ...
      <div v-if="message.reasoning_content" class="reasoning-box">
        <a-collapse
          ...
          @change="reasoningActiveKey = $event"
          :bordered="false"
        >
          ...
          <a-collapse-panel
            key="show"
            :header="message.status == 'reasoning' ? 'thinking...' : 'reasoning process'"
      ...
      <div
        v-else-if="message.status === 'searching' && isProcessing"
        class="searching-msg"
      >
        <i>searching...</i>
      </div>
      ...
      <div
        v-else-if="message.status === 'generating' && isProcessing"
        class="searching-msg"
      >
        <i>generating...</i>
      </div>
      ...
      <div v-else-if="message.reasoning_content" class="empty-block" />
      ...
      <div v-else class="err-msg" @click="$emit('retry')">
        Request error, please try again: {{ message.message }}
      </div>

      <div v-if="message.isStoppedByUser" class="retry-hint">
        You stopped the generation, please edit the question again
        <span class="retry-link" @click="emit('retryStoppedMessage', message.id)"
          >Edit the question again</span
        >
      </div>
...
.message-md .md-editor-preview-wrapper {
  ...
  font-family: -apple-system, BlinkMacSystemFont, "Noto Sans SC", "PingFang SC",
    "Noto Sans SC", "Microsoft YaHei", "Hiragino Sans GB", "Source Han Sans CN",
  ...
  code {
    font-size: 13px;
    font-family: "Menlo", "Monaco", "Consolas", "PingFang SC", "Noto Sans SC",
      "Microsoft YaHei", "Hiragino Sans GB", "Source Han Sans CN", "Courier New",
      monospace;
    line-height: 1.5;
